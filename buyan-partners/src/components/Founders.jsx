...
const Founders = () => {
  ...
  const DEFAULT_IMAGE =
    "https://images.unsplash.com/photo-1633332755192-727a05c4013d?auto=format&fit=crop&q=80&w=1000";
  ...
  const handleNext = (e) => {
    e?.stopPropagation();
    setDirection(1);
    const currentIndex = founders.items.findIndex(
  ...
  const handlePrev = (e) => {
    ...
    const prevIndex =
      (currentIndex - 1 + founders.items.length) % founders.items.length;
  ...
  return (
    <section
      id="founders"
      className="py-12 md:py-24 bg-gray-50 dark:bg-slate-950 transition-colors duration-300 relative"
    >
      ...
      <AnimatePresence>
        {selectedId && selectedFounder && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
            ...
            <button
              onClick={handlePrev}
              className="absolute left-2 md:left-8 top-1/2 -translate-y-1/2 z-[110] p-3 rounded-full bg-white/10 hover:bg-white/20 text-white transition-all backdrop-blur-md border border-white/10 hidden md:block"
            >
              <ChevronLeft size={32} />
            </button>
            <button
              onClick={handleNext}
              className="absolute right-2 md:right-8 top-1/2 -translate-y-1/2 z-[110] p-3 rounded-full bg-white/10 hover:bg-white/20 text-white transition-all backdrop-blur-md border border-white/10 hidden md:block"
            >
            ...
            <motion.div
              ...
              animate="visible"
              exit="exit"
              className="bg-white dark:bg-slate-900 w-full max-w-5xl h-[70vh] md:h-[600px] overflow-hidden rounded-3xl shadow-2xl relative z-[105] flex flex-col md:flex-row"
            >
              <button
                onClick={() => setSelectedId(null)}
