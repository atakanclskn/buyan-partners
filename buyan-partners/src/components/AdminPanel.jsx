...
const AdminPanel = ({ onClose }) => {
  ...
  // --- GENEL HANDLERLAR (Tüm alt bileşenler bunu kullanır) ---
        ...
        <div className="max-w-7xl mx-auto bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 p-8 min-h-[500px] relative">
          ...
          {activeTab === 'services' && (
            <ServicesEditor 
              ...
              onDetailChange={(sIdx, dIdx, k, v) => {
                 // Servis Detayı karmaşık olduğu için burada inline halletmek yerine logic'i yukarı taşıyabilirdik ama
                 // Şimdilik ServicesEditor içinde basitçe halledip buraya data olarak aktarıyoruz.
                 // ServicesEditor kendi içinde prop olarak aldığı onArrayChange ve onChange'i kullanıyor.
                 // Detay güncelleme mantığı ServicesEditor içinde self-contained.
              }}
              // Aslında ServicesEditor'a formData setleyicisi lazım detaylar için.
              // En temizi: ServicesEditor'a setFormData'yı değil, özel bir update fonksiyonu geçmek.
          ...
          {activeTab === 'contact' && (
            <ContactEditor 
              ...
              onInfoChange={(k, v) => setFormData(prev => ({ ...prev, contact: { ...prev.contact, info: { ...prev.contact.info, [k]: v } } }))}
          ...
          {activeTab === 'footer' && (
            <FooterEditor 
              ...
              onGeneralChange={(k, v) => setFormData(prev => ({ ...prev, general: { ...prev.general, [k]: v } }))}
              onArrayChange={(arr, i, k, v) => {
